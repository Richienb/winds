...
module.exports = async () => {
    if (await commandExists("powershell")) {
        const { stdout: res, stderr: error } = await execa("powershell", ["-ExecutionPolicy", "Bypass", "Get-Process | Where-Object {$_.MainWindowTitle -ne \"\"} | Select-Object MainWindowTitle"])
        if (error) throw new Error(error)
        return res.split("\r\n").slice(3, -2).map((value) => value.trimRight())
    }

    if (await commandExists("cmd")) {
        const { stdout: res, stderr: error } = await execa("tasklist", ["/v", "/fo", "csv"])
        if (error) throw new Error(error)
